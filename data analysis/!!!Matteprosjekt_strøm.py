...
if path.exists(mittDatasett):
    ...
    pris = pd.read_csv("C:\\Users\\adria\\OneDrive\\Visual studio Python\\Koding prosjekt\\Matte\Datasetter\\PowerPrice.txt", sep=";") # #Endre denne basert på hvilken plassering datasettet har i filene dine her og for den neste også.
    flow = pd.read_csv("C:\\Users\\adria\\OneDrive\\Visual studio Python\\Koding prosjekt\\Matte\Datasetter\\Flow.txt", sep=";") #bruker ordet flow for produksjon og forbruk. Endre PATH basert på hvilken plassering datasettet har
...
while len(valg) > 0:
    ...
    if valg[0] in dataer:
        ...
        if tid in ['TIME','UKE','MND']:
            ...
            behandling()
            plt.subplot(2,2,x)
            datasett.groupby([tid.capitalize()])[data.capitalize()].mean().plot(label=(overskrift), legend=True,layout='constrained').set_xticks(label, labels = akse)
            plt.ylabel(ylabel)
