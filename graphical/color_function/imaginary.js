...
// Fn = ( ( ( 1+Math.sqrt(5) )**(x+y) ) - ( ( 1-Math.sqrt(5) )**(x+y) ) ) / ( ((2)**(x+y)) * Math.sqrt(5) )

//TODO: Fargen bestemmes ut i fra vinkelen?
...
//TODO! Make it possible for a change saturation and hue function,
//in addition to changing hue and sat in winInit
...
function get_cursor_position(canvas, event) {
  ...
  if (event.type == "mousedown") {
    ...
    list1 = [down_x];
    list2 = [-down_y];
    // absolute_width_square = Math.floor(canvas.width/size)
    // absolute_heigth_square = Math.floor(canvas.height/size)
    ...
    console.log("down", down_y);
    console.log("up", up_y);
    // current_x = ( (event.clientX - rect.left) / absolute_width_square ) + size_lower
    // current_y = ( (event.clientY - rect.top ) / absolute_width_square ) + size_lower
    scaled = true;
    if (event.ctrlKey) {
      scaled = false;
    }
    ...
    // tegnFirkant(x1,y1,x2,y2,farge='blue',fill=true)
    // tegnBrukXY(x1,x2,y1,y2)

    // tegnBrukXY( Math.abs(list1[0]*(size/20)), Math.abs(current_x * (size/20)),  Math.abs( (size -  (list2[0] * (size/20)) )  ), Math.abs(size -current_y* (size/20)), 'blue', false)

    tegnBrukXY(list1[0], list1[1], list2[0], list2[1]);
    // tegnBrukXY(list1[0], list1[1], Math.abs(size -list2[1]), Math.abs(size-list2[0]));
    // tegnBrukXY(x1,x2,y1,y2)
    console.log("list1:", list1);
    console.log("list2:", list2);
    // tegnFirkant( list1[0],  (list2[0]-difference)  , current_x, (-current_y-difference), 'blue', false)

    // console.log(list1, list2)
    // tegnBrukXY(0, 20, 10, 20-0);
    create_squares();
    draw_squares();
  }
}
...
function winInit() {
  ...
  // Kobler canvas i html sammen med tegnepakka. Viktig ved lokal koding
